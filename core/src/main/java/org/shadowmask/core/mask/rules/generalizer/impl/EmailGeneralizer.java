...
public class EmailGeneralizer implements Generalizer<String, String> {
  ...
  @Override
  public String generalize(String email, int hierarchyLevel) {
    if (email == null) {
      return null;
    }

    if (hierarchyLevel > ROOT_HIERARCHY_LEVEL || hierarchyLevel < 0) {
      throw new MaskRuntimeException("Root hierarchy level of IPGeneralizer is " + ROOT_HIERARCHY_LEVEL +
    ...
    if (subs.length != 2) {
      throw new MaskRuntimeException("Invalid input email to generalize:" + email);
    }
